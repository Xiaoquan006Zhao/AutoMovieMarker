...
exports.handler = async (event, context) => {
  ...
  await template.templatedQuery(event, queryDeleteClipEmotion, [movie_id]);

  // Delete clips records.
  const queryDeleteClips = "DELETE from clips WHERE movie_id = ?";
  await template.templatedQuery(event, queryDeleteClips, [movie_id]);

  // Delete movie record.
  const queryDeleteMovie = "DELETE from movies WHERE movie_id = ?";
  const data = await template.templatedQuery(event, queryDeleteMovie, [
    movie_id,
  ]);
